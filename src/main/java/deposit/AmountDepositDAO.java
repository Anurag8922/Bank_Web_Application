...
public class AmountDepositDAO
{
	public double amountDeposit(HttpServletRequest req,HttpServletResponse res)
    {
    ...
    ("insert into AmountTransaction(AccountNo,Description,TransactionAmount,Time) values(?,?,?,?)");
            	    ps1.setLong(1,Long.parseLong(req.getParameter("accountno")));			   
            		ps1.setString(2,req.getParameter("Deposit"));
            		ps1.setDouble(3,Double.parseDouble(req.getParameter("amount")));
            	    ps1.setString(4,new Date().toString().substring(11,19));
            		if(ps1.executeUpdate()>0)
            		{
            		    currentBalance=rs.getDouble(16)+Double.parseDouble(req.getParameter
            				                                                             ("amount"));
            		    PreparedStatement ps2=con.prepareStatement
            				        ("update BankMainTable set AccountBalance=? where AccountNo=?");
