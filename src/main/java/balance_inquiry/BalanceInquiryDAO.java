...
public class BalanceInquiryDAO
{
	public ArrayList<BeanClass> balanceInquiry(HttpServletRequest req,HttpServletResponse res)
    {
       ...
       try
       {
    	   ...
    	   PreparedStatement ps=con.prepareStatement
    			    ("select * from BankMainTable where AccountNo=? and Password=?");
    	   ...
    	   if(rs.next())
    	   {
               ...
               ("select * from AmountTransaction where AccountNo=? order by TransactionDate desc");
              	    ...
              	    while(rs1.next())
              	    {
              	         ...
              	         bc.setDescription(rs1.getString(2));
              	    	 bc.setTransactionAmount(rs1.getDouble(3));
              	    	 bc.setTime(rs1.getString(4));
              	    	 Date td=rs1.getDate(5);
            		     String TransDate=td.toString().substring(8,10)+"-"+td.toString()
            		                              .substring(5,7)+"-"+td.toString().substring(0,4);
            		     bc.setOpeningDate(TransDate);
            		     al.add(bc);
              	    }
               }
           	   else
	...
	public BeanClass getCustomerDetails(HttpServletRequest req,HttpServletResponse res)
    {
       BeanClass bc=null;
       try
       {
    	   ...
    	   if(rs.next())
    	   {
    		   ...
    		   String openingDate=od.toString().substring(8,10)+"-"+od.toString().substring(5,7)+
    				                                              "-"+od.toString().substring(0,4);
    		   ...
    		   File f=new File
               ("S://Eclipse Workspace//My Projects//bank_of_india_application//src//main"
    		                                                                  +"//webapp//"+photo);
